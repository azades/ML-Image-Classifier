...
def predict(image_tensor, model, device, cat_to_name, top_k):
    ...
    log_probabilities = model.forward(torch_image)

    # Convert to linear scale
    linear_probabilities = torch.exp(log_probabilities)

    # Find the top 5 results
    top_probabilities, top_labels = linear_probs.topk(top_k)
...
def main():
    ...
    top_probabilities, top_labels, top_flowers = predict(image_tensor, model, 
                                                 device, cat_to_name,
                                                 args.top_k)
